...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: item } = await admin.from('transfer_items').select('storage_path, original_filename, type, deleted_at').eq('id', body.transferItemId).eq('transfer_space_id', body.transferSpaceId).maybeSingle();
    if (!item || item.type !== 'file' || item.deleted_at || !item.storage_path) return error('文件不存在。', 404, 'ITEM_NOT_FOUND');
