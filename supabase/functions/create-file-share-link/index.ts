...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: item, error: itemError } = await admin.from('transfer_items').select('id, type, transport, deleted_at, expires_at, uploader_device_id, uploader_anonymous_user_id').eq('id', body.transferItemId).eq('transfer_space_id', body.transferSpaceId).maybeSingle();
